...
{
    /// <summary>
    /// 良品批次（good_lots）資料表的 Repository 介面。
    /// </summary>
    public interface IGoodLotRepository
    {
        /// <summary>取得所有良品批次記錄。</summary>
        /// <returns>GoodLot 集合（可能為空）。</returns>
        IEnumerable<GoodLot> GetAll();

        /// <summary>依主鍵查詢單筆良品批次。</summary>
        /// <param name="id">記錄主鍵 ID。</param>
        /// <returns>找到時回傳 <see cref="GoodLot"/>，否則回傳 null。</returns>
        GoodLot GetById(long id);

        /// <summary>依批次資訊 ID 查詢良品批次清單。</summary>
        /// <param name="lotsInfoId">外鍵，對應 lots_info.id。</param>
        /// <returns>符合條件的 GoodLot 集合（可能為空）。</returns>
        IEnumerable<GoodLot> GetByLotsInfoId(int lotsInfoId);

        /// <summary>新增一筆良品批次，回傳新記錄的主鍵 ID。</summary>
        /// <param name="entity">要新增的 GoodLot 實體。</param>
